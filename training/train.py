...
def train_pipeline(
    ...
    if sm_channel_train:
        # SageMaker Mode: Data is mounted at SM_CHANNEL_TRAIN
        # The input s3_uri (folder) is mounted, so we need to find the csv file in it
        print(f"SageMaker Mode 감지: {sm_channel_train}")

        # SM_CHANNEL_TRAIN is a directory. Find the first csv file.
        files = [f for f in os.listdir(sm_channel_train) if f.endswith(".csv")]
        if not files:
            # If no csv found, maybe the input IS the file (depends on S3Uri in InputDataConfig)
            # But usually it's a prefix/directory. Let's assume it's a file path if S3 prefix was a file.
            pass
            # Actually, simpler: join path with filename if we passed filename in lambda
            # BUT here we assume the folder is mounted.
            # Let's search recursively or just pick one.

        if files:
            csv_path = os.path.join(sm_channel_train, files[0])
            print(f"데이터 파일 경로 설정: {csv_path}")
        else:
            # If explicit file path was passed via args but we are in SM?
            # Usually strict SM usage ignores args.csv_path if using channels.
            print(
                f"경고: {sm_channel_train} 내에 CSV 파일을 찾을 수 없습니다. 인자로 전달된 경로({csv_path})를 시도합니다."
            )

    # S3 경로 처리 (Local Mode Fallback)
    ...
    for epoch in range(epochs):
        ...
        avg_train_loss = (
            running_loss / len(train_loader) if len(train_loader) > 0 else 0
        )
        ...
        print(
            f"  Epoch [{epoch+1}/{epochs}] | Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f} | Val Acc: {val_acc:.2f}%"
        )
    ...
    # SageMaker Model Output Dir
    sm_model_dir = os.environ.get("SM_MODEL_DIR")
    if sm_model_dir:
        save_dir = Path(sm_model_dir)
        print(f"SageMaker 모델 저장 경로: {save_dir}")
    else:
    ...
    # 9. 레지스트리 업로드 (Local Mode Only)
    # SageMaker에서는 자동으로 model.tar.gz를 S3에 올리므로, 이 단계는 로컬 실행 시에만 필요
    if upload_s3 and not os.environ.get("SM_MODEL_DIR"):
        print(f"레지스트리에 업로드 중 ({version})...")
        try:
            registry = ModelRegistry()
            # Model
            model_s3 = registry.upload_model(
                str(model_path), "efficientnet-b0-cls", version
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-upload", action="store_true", help="Skip uploading to S3 Registry"
    )
